...
[...Array(numberOfExecutions)].reduce(async(results) => [
  ...
  .then(() => {
    ...
    text += `It takes an average of ${avgTimeToPostAll} milliseconds to post ${numberOfPosts} ${dimension}X${dimension} ${cellShape} ${algorithm} maze\n`;
    text += `The Fastest Post Time: ${theMinTime}\n`;
    text += `The Slowest Post Time: ${theMaxTime}\n`;
    text += `The Overall Average Median Post Time: ${avgMedianTime}\n`;
    text += `The Overall Average Mean Post Time: ${avgMeanTime}\n`;

    return fs.writeFile(`./lib/data/results/${dimension}X${dimension}-${cellShape}-${algorithmName}-metrics-${numberOfPosts}posts-executed-${numberOfExecutions}times.txt`, text, { encoding: 'utf8' })
      .then(()=> {
